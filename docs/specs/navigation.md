...
`Analyze`, `History`, `Presets`, `Settings`, `Feedback`, `Language`,
`Theme`, `Analytics`. The design's Analyze nav bar also carries a share
icon, but every nav bar the app renders is title only, with an optional
back affordance on a screen that has somewhere to go back to (`Feedback`,
`Language`, `Theme`, and `Analytics` are the only ones that do). Leaving
the share icon out is deliberate, not an omission: a later session that
opens the design file will still find it there.
