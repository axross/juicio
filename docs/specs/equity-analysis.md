...
This document describes what the design specifies for the Analyze screen;
see [equity-breakdown.md](./equity-breakdown.md) for its Equity Breakdown
sheet. The Analyze tab's empty state, the board's own empty state, and the
`Players` section heading above it, built by issue #64, are built and
shipped — and, as of issue #87, so is the players list itself, both its row
kinds and its swipe-to-delete gesture, as the sections below now describe.
**As of issue #99**, so are the board's own populated state, both card
pickers' exclusion of a card already spoken for elsewhere, and the Analyze
toast — see The Board, The Board Input Sheet, The Toast, and The Players
Section below. **As of issue #102**, so is a row's own result figure and,
for a hand-range player, its chevron and the Equity Breakdown sheet those
open — see Player Kinds below and [equity-breakdown.md](./equity-breakdown.md)
for what is shipped there and what still is not. **As of issue #103**, so is
the equity engine itself, and with it the Calculating and Calculated screen
...
List, and Player Kinds below, and [equity-breakdown.md](./equity-breakdown.md),
for exactly what changed. **As of issue #143**, a row's own result figure
updates live throughout the calculation instead of staying hidden until it
settles, and a hand-range row's chevron and Equity Breakdown detail press
become reachable the moment its own row shows any number, live or settled
alike — see The Players List and Player Kinds below,
[equity-breakdown.md](./equity-breakdown.md), and
...
note). The code for this domain sits under `src/features/evaluations/` — the
one name this project gives it other than Analyze.
...
existing one — see [conventions/copy-conventions.md](../conventions/copy-conventions.md)'s
...
- **Empty** — no players yet, built and shipped: the board's five empty
  ...
  settled in [conventions/copy-conventions.md](../conventions/copy-conventions.md).
...
preview: pressing it opens the Equity Breakdown sheet (see
[equity-breakdown.md](./equity-breakdown.md)), fires the same `primaryAction`
haptic the preview's own edit press already does
...
- A **hand range** — rendered as a 13×13 dot-matrix preview next to a card
  ...
  Every range player this change can build is ad hoc rather than from a
  saved preset — there is no preset store yet. Its result figure is a real,
  computed one — the design's own *averaged* result across the range — once
  the table holds two or three players and the running evaluation has
  reported one for that player (issue #103); before that, it
  ...
  opens the Equity Breakdown sheet, unlike a hole-cards player's; see
  [equity-breakdown.md](./equity-breakdown.md).
