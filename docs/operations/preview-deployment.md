...
Every Android build this project produces — Preview, Release, and CI
verification alike — runs with Gradle's own local build cache
(`org.gradle.caching=true`) and parallel task execution
(`org.gradle.parallel=true`) turned on in the generated
`android/gradle.properties`. Neither flag was set anywhere before: `android/`
is Continuous-Native-Generation output, gitignored and regenerated by every
`expo prebuild`, so there was no committed `gradle.properties` for either flag
to live in.

The same regeneration problem the ABI restriction above solves reapplies
here, and the same answer — a config plugin declared in
[`app.json`](../../app.json)'s `plugins` array, rather than a hand edit to
the generated file — was reached for the same reason: a hand edit to
`android/gradle.properties` is silently reverted the next time anything runs
`expo prebuild`. This one cannot go through `expo-build-properties` itself,
though: that package's Android configuration surface is a fixed, typed list
...
`./gradlew assembleDebug` fail outright, with Gradle's own
configuration-cache report naming several `node` processes the generated
`android/app/build.gradle` (Expo's own output, not this plugin) starts at
configuration time — `require('expo/scripts/resolveAppEntry')`,
`require.resolve('react-native/package.json')`, and similar — as
unsupported under configuration cache
([Gradle's own requirements doc](https://docs.gradle.org/9.3.1/userguide/configuration_cache_requirements.html#config_cache:requirements:external_processes)).
That is a property of Expo's generated Android project at the version this
project pins, not of this plugin, so enabling configuration cache would need
either an Expo/React Native change upstream or an unrelated version bump to
this project's own Gradle/Android Gradle Plugin pins — outside this change's
scope. It is left off for now; see
[issue #201](https://github.com/axross/juicio/issues/201) for the record of
that outcome.
