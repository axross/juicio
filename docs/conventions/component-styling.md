...
returns at the top level. Two things it does not reach:
...
| A layout `View` wrapping one interactive element — `TextField` | Its props inherit the `TextInput`'s own ([`text-field.tsx`](../../src/features/feedback/ui/text-field.tsx)), since the input is this field's identity, not the `View` that lays its label, hint, and error around it. That inherited `style` is what reaches the `TextInput` directly (`style={[styles.input, style]}`); the wrapping `View` root takes no caller override of its own at all. This is the one row in this table where the element a caller's `style` actually reaches is not the component's own literal JSX root — say so plainly rather than leaving a reader to assume the general rule above applies unmodified here. |
...
| A file-private subcomponent | Same rule, but its one caller is in the same file, so it takes `style` only when that caller passes one. `FanCard` does ([`cards-pane.tsx`](../../src/shared/ui/cards-pane/cards-pane.tsx)); `BoardSlot`, `NewPlayerRow`, `PreviewSlot`, `FanArc`, `GridCellComponent`, `ShorthandChip`, and `Tab` do not. |
