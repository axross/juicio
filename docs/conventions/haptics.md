...
| `toggleOff` | `impactAsync(ImpactFeedbackStyle.Light)` | `Toggle_Off` | Deselecting a rank-pair grid cell, clearing an armed card/range input sheet preview slot by tapping it again, and pressing a hand-range shorthand chip that deselects every one of its own rank pairs ([specs/hand-ranges.md](../specs/hand-ranges.md)). |
