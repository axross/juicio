...
| `toggleOn` | `impactAsync(ImpactFeedbackStyle.Light)` | `Toggle_On` | Selecting a rank-pair grid cell, filling or overwriting a card/range input sheet preview slot, and turning the Settings Analytics screen's tracking switch on (issue #211, [`switch-row.tsx`](../../src/features/settings/ui/switch-row.tsx)) — this app's first boolean switch control; Settings' Theme row is a radio, not a toggle. |
| `toggleOff` | `impactAsync(ImpactFeedbackStyle.Light)` | `Toggle_Off` | Deselecting a rank-pair grid cell, clearing a card/range input sheet preview slot by tapping its own focused, filled slot again, and turning that same Analytics switch off. |
| `dragStart` | `impactAsync(ImpactFeedbackStyle.Medium)` | `Drag_Start` | Picking up a player or history row's swipe-to-delete gesture ([specs/equity-analysis.md](../specs/equity-analysis.md), [specs/calculation-history.md](../specs/calculation-history.md)) — shipped for the Analyze players list (issue #87, [`player-row.tsx`](../../src/features/evaluations/ui/player-row/player-row.tsx)) and, since, for History's own row too (issue #180, [`history-entry-row.tsx`](../../src/features/history/ui/history-entry-row/history-entry-row.tsx)). **Also fires for the player row's own long-press-to-drag reorder gesture** (issue #153): a second, independent gesture on that same row reusing this event rather than `longPress` below — `player-row.tsx`'s own `Gesture.Exclusive` guarantees the two gestures never activate at once, so this is never two overlapping pickups; History's row has no reorder gesture, so only its swipe fires this event there. |
| `dragEnd` | `impactAsync(ImpactFeedbackStyle.Light)` | `Gesture_End` | Releasing that same swipe once it settles into one of three outcomes — rests closed, rests revealed, or commits to delete — that both `player-row.tsx` (issue #87) and `history-entry-row.tsx` (issue #180) share; each also reuses this event for a tap on its own revealed delete panel, which concludes that same swipe interaction without a fresh release to fire from. Each row's own accessibility action fires no haptic at all — it deletes without a swipe ever having started, so there is no gesture for this event to conclude. **Also fires once the player row's long-press-to-drag reorder gesture above releases** (issue #153), for the same reason `dragStart` above does; History's row has no reorder gesture, so this only applies to Analyze. |
| `sheetOpen` | `impactAsync(ImpactFeedbackStyle.Light)` | `Gesture_Start` | Presenting the card/range input sheet ([specs/hand-ranges.md](../specs/hand-ranges.md)) or the Equity Breakdown sheet ([specs/equity-breakdown.md](../specs/equity-breakdown.md)) — `src/shared/ui/bottom-sheet/bottom-sheet.tsx` fires this at the entrance spring's first arrival at the open position (or immediately, under reduce motion, where there is no animation to arrive), never on the frame the entrance is scheduled — so any caller of that shared component gets it for free. It used to wait for the entrance animation to report it has settled instead; that read as out of sync with the sheet's own visual landing, since a spring's real settle time runs roughly 1.5× its nominal duration (`src/core/motion/tokens.ts`'s own doc comment on `motionSpringConfig`) — the maintainer felt the gap on-device (issue #101) — so this now fires on the spring's first crossing of the open position, which its deliberate slight overshoot guarantees happens, rather than waiting for the tail of that overshoot to decay. |
...
#87); each row above now also names History's own row as a second shipped
caller for both (issue #180) — neither event has an anticipated surface
left to name.
