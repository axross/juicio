...
| `dragTick` | `selectionAsync()` | `Segment_Frequent_Tick` | Dragging across the hand-range grid's 13×13 cells and crossing into a new combo ([specs/hand-ranges.md](../specs/hand-ranges.md)). Anticipated: no caller is wired yet, since the grid itself is not built. |
