...
fn main() -> ExitCode {
    ...
    let handles: Vec<std::thread::JoinHandle<Vec<HashMap<CardPair, Tally>>>> = (0..threads)
        .map(|index| {
            let part = evaluator.partition(threads, index, index + 1);
    ...
    for handle in handles {
        match handle.join() {
            Ok(part) => merge(&mut tallies, part),
            Err(_) => {
                eprintln!("a worker thread panicked; the totals below would be partial.");
...
fn parse(args: &[String]) -> Result<(Vec<Card>, Vec<HandRange>), String> {
    ...
    if rest.len() < 2 {
        return Err(
            "usage: multi-thread [<board>] <range> <range> [<range>]  (e.g. Qs8d2h JJ+ A2s+)"
                .to_string(),
        );
    }
...
fn merge(into: &mut [HashMap<CardPair, Tally>], part: Vec<HashMap<CardPair, Tally>>) {
    for (index, player) in part.into_iter().enumerate() {
        for (pair, tally) in player {
            ...
            target.share += tally.share;
            target.total += tally.total;
        }
    }
}
