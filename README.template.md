...
An app that helps with playing Texas hold'em poker and reviewing that play afterwards.
...
| ---- | ---- |
| Language | TypeScript |
| App framework / runtime | Expo |
| Package manager | npm |
| Linting & formatting | ESLint / Prettier |
| Unit tests | Jest <!-- INIT:OPTIONAL key=UNIT_TESTS — fill the token OR delete this row if the project has no unit suite. --> |
| E2E tests | Maestro <!-- INIT:OPTIONAL key=E2E_TESTS — fill the token OR delete this row if the project has no e2e suite. --> |
| Data / content layer | Drizzle ORM over expo-sqlite <!-- INIT:OPTIONAL key=DATA_LAYER — fill the token OR delete this row if the project has no data/content layer. --> |
| Hosting | Firebase App Distribution <!-- INIT:OPTIONAL key=HOSTING — fill the token OR delete this row if the project has no hosting platform yet. --> |
...
3. Production build and start: `npm run build`, then `npm run android`
