...
describe('tapSlot()', () => {
  ...
  describe('under the left-packed fill policy', () => {
    ...
    it('clears the focused slot, shifts every card right of it one place left, and keeps focus', () => {
      const start: CardsPaneState = {
...
describe('the left-packed policy over a sequence of operations', () => {
  ...
  const OPERATIONS: readonly Operation[] = [
    ...
    { kind: 'tap', slotIndex: 2 }, // clear a card mid-run
    { kind: 'tap', slotIndex: 4 }, // clamped back to the first empty slot
  ];
