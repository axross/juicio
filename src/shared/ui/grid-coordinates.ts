 ...
 * `RankPair`'s own construction and notation rules.
 *
 * **lives at `ui/`'s own top level, not inside either of its two
 * consumers' own directories.** `hand-range-pane.tsx` is one consumer, in
 * `hand-range-pane/` beside it; `rank-pair-grid/rank-pair-grid.tsx` (the
 * players list's own dot-matrix preview) is a second, genuinely
 * independent consumer in a different directory — the exact shape
 * docs/conventions/
 ...
 * graph actually shows, so it lives at `ui/`'s own top level instead, per
 * that document's rule.
 *
 ...
 * this module is not coupled to one directory at all; `grid-
