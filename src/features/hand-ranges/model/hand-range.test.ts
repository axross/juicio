...
import { handRangeCardPairCount, type HandRange } from './hand-range';
...
describe('handRangeCardPairCount()', () => {
  it('is 0 for an empty range', () => {
    expect(handRangeCardPairCount(new Set())).toBe(0);
  });

  it('sums a single pocket pair to 6', () => {
    expect(handRangeCardPairCount(new Set([rankPairKey(rankPair('A', 'A', true))]))).toBe(6);
  });
  ...
  it('sums a suited hand and its offsuit counterpart to 16', () => {
    ...
    expect(handRangeCardPairCount(range)).toBe(16);
  });
  ...
  it('sums the full 13×13 grid to 1326 card pairs', () => {
    ...
    expect(handRangeCardPairCount(fullGrid())).toBe(1326);
  });
});
