 ...
 * own `CardsPaneSlots` names — read off `HoldingInputState['holeCards']`
 ...
 * structurally identical types, so a caller handing this hook's own
 ...
 * `HandRangePane`'s own sibling leaf hook (see
 ...
 * current pair and its setter, exactly `useState`'s own shape, so this
 * state is reusable across whatever screen wants a hole-card selection of
 * its own, independent of any other caller's instance.
 *
 ...
 * it up here would duplicate `initialFocusedSlot`'s own derivation rather
 * than reuse it.
 */
