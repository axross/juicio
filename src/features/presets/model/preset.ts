 ...
 * Preset section, docs/glossary.md's Tag Axis entry. fixed by the spec; a
 * plain literal union, not derived from a values catalog — the canonical
 * `(axis, value)` catalog itself lives at
 * `@/features/presets/adapter/seed-tag-catalog` (the one place it is
 * spelled out), and this type only fixes the four axis *keys*, which stay
 * structurally fixed by the spec regardless.
 */
export type TagAxis = 'position' | 'players' | 'stack' | 'action';

/**
 * a Preset's selected values across all four tag axes. each axis is
 * multi-select and may be left with no selected value at all — an empty
 * array, never `undefined`, so every one of the four axes is always
 * present on a `Preset`. a value is `string`, not a compile-time literal
 * union of the 17 known values: only the axis names themselves are
 * structurally fixed (`TagAxis` above), while their individual values are
 * bootstrap-seeded data the persistence layer validates against at
 * runtime, not the type system.
 */
 ...
 * no matching stored Preset, rather than a null/undefined placeholder.
 */
