...
/**
 * a History Entry's own holding representation —
...
export type HistoryEntryPlayer = {
   ...
   * calculation was saved — a frozen copy of that one rendered string
   ...
   * identity, which stays this array's own position — `name` is purely an
   * added display label alongside it, never used to determine order or
   * identity.
   */
 ...
 * instant a running equity evaluation reaches its successful result; never
 * constructed for any other evaluation outcome.
 */
export type HistoryEntry = {
   ...
   * own `Board` type, per this feature's own Assumptions. */
